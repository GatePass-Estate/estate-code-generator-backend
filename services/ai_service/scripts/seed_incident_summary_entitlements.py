#!/usr/bin/env python3
"""
Seed incident-summary AI features and grant them to one estate.

Creates (if missing):
  - ai_feature rows for
      incident_summary_basic (result page only)
      incident_summary_basic_tier2 (in-house topic modelling)
      incident_summary_basic_tier3 (LLM, includes in-house)
...
Does not touch anomaly catalog rows or grants owned by
``seed_anomaly_case_summary_entitlements.py``.

Paste the estate UUID below, then run from ``services/ai_service``::

    python scripts/seed_incident_summary_entitlements.py
...
AI_SEEDS: list[dict] = [
    {
        "feature_key": "incident_summary_basic",
        "name": "Incident Report Result Page",
        "description": (
            "Overview and report list for an estate date window. "
            "Does not include AI-generated summaries."
        ),
        "is_free": False,
    ...
    {
        "feature_key": "incident_summary_basic_tier2",
        "name": "Incident Report Summary (In-house)",
        "description": (
            "TF-IDF/NMF topic-modelling summary for the selected estate date window."
        ),
        "is_free": False,
        "feature_unit_price": 500,
        "delete_catalog": True,
    ...
    {
        "feature_key": "incident_summary_basic_tier3",
        "name": "Incident Report Summary (LLM)",
        "description": (
            "Includes the in-house topic report plus a third-party LLM narrative."
        ),
        "is_free": False,
        "feature_unit_price": 500,
        "delete_catalog": True,
...
async def _get_or_create_ai_grant(
    ...
    *,
    ...
    if existing:
        print(
            f"Reusing estate_ai_feature {feature['feature_key']} id={existing[0]['id']}"
        )
        return existing[0]
    ...
    print(
        f"Created estate_ai_feature {feature['feature_key']} id={created['id']}"
    )
    return created


async def seed(estate_id: str) -> None:
    """Seed incident-summary AI features, prices, and estate grants."""
...
async def reverse(estate_id: str) -> None:
    """Remove this script's grants; drop only the new tier catalog rows."""
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        for spec in AI_SEEDS:
            ...
            if not spec.get("delete_catalog"):
                continue
            await _delete_all(
                client,
                "api/v1/revenue/featureunitprice/search",
        ...
        print(f"Reversed incident-summary grants for estate_id={estate_id}")


async def main() -> None:
    # Current local estate used for incident / spatial result-page work.
