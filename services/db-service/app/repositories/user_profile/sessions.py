...
class SessionsRepository:
    """
    Repository to operate on sessions table.
    """
    ...
    async def create(self, request: CreateRequest) -> CreateResponse:
        ...
        try:
            ...
            message = "Database error in creating the session"
            logger.exception(message)
            raise DatabaseError(message) from e
    ...
    async def search(
        ...
        for key, info in request.model_fields.items():
            ...
            elif key == "last_active_after":
                query = query.where(
                    TableModel.last_active_at >= request.last_active_after
                )
    ...
    async def delete_all_for_user(self, user_id: UUID4) -> None:
        ...
        """
        try:
            # Query all active sessions for the given user.
            query = select(TableModel).where(
                TableModel.is_deleted == False,  # noqa E712
            ...
            # Soft-delete each session record.
            ...
            message = (
                "Database error in deleting all sessions for user %s" % user_id
            )
            logger.exception(message)
            raise DatabaseError(message) from e
        except Exception as e:
