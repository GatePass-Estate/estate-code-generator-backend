...
async def search(
    ...
    visit_time: datetime.datetime | None = None,
    ...
    Filter by ``hashed_code`` for second-level visit lists. Rows include
    denormalized ``resident_fullname``. Default order is newest first
    (``ascending=false``).
