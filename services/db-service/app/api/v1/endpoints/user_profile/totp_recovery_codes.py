...
from app.services.user_profile.totp_recovery_codes import (
    TotpRecoveryCodesService as Service,
)
...
@router.delete(
    ...
    responses={
        500: {"description": "Internal server error"},
        200: {"description": "Deleted all recovery codes for the user"},
    },
    description="Delete all recovery codes for a user",
)
async def delete_all_for_user(
    ...
    """
    Soft-deletes all recovery codes belonging to the specified user.

    Arguments:
        user_id: The unique ID of the user whose recovery codes to DELETE.
    ...
    try:
        ...
        logger.exception(
            "An unexpected error happened while deleting all recovery codes "
            "for the user"
        )
        raise HTTPException(
            status_code=500, detail="Internal server error"
...
async def search(
    ...
    Arguments:
        ...
        is_used (bool): Filter by used status.
